...
{
    $query  = "SELECT komentar.keterangan, komentar.created_at, users.name FROM komentar INNER JOIN users ON komentar.user_id = users.id WHERE komentar.artikel=$id ORDER BY komentar.id DESC";
    return result($query);
}
